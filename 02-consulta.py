...
cursor.execute("""
   SELECT COUNT(*)
   FROM requisito
   WHERE id_anuncio = 1 AND tipo = 'Obrigatorio'
""");
...
cursor.execute("""
   ...
   WHERE st.id_anuncio = 1 AND st.status = 'cumprido' AND req.tipo = 'Obrigatorio'
   GROUP BY u.nome
""")
...
cursor.execute("""
   ...
   FROM
      ...
      JOIN requisito req
         ON st.id_anuncio = req.id_anuncio AND st.titulo = req.titulo
      JOIN usuario u ON st.email_candidato = u.email
   WHERE st.id_anuncio = 1 AND st.status = 'cumprido' AND req.tipo = 'Opcional'
   GROUP BY u.nome
""")
